...
export const createMinimalTemplate = async (frameworkLower, projectName) => {
  
  console.log(chalk.gray(`\n📦 Copying minimal template for ${frameworkLower}...\n`));

  try {
    // Step 4a: Copy Minimal Template to project folder
    const templatePath = `templates/${frameworkLower}-minimal`;
    ...
    if (error.message.includes('not found')) {
      ...
      console.log(chalk.yellow(`\n💡 Something went wrong copying the minimal template for ${frameworkLower}`));
    }
